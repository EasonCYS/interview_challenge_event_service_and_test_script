...
if p_order = 'desc' then
	SET @s=CONCAT('SELECT * FROM event WHERE user_id =', p_user_id,' AND end_time > "',p_end_time_range,'" ORDER BY ', p_sort_key , ' desc;');
	PREPARE stmt1 FROM @s;
	EXECUTE stmt1;
	DEALLOCATE PREPARE stmt1;
else
	SET @s=CONCAT('SELECT * FROM event WHERE user_id =', p_user_id,' AND end_time > "',p_end_time_range,'" ORDER BY ', p_sort_key , ' asc;');
	PREPARE stmt1 FROM @s;
	EXECUTE stmt1;
	DEALLOCATE PREPARE stmt1;
end if;
